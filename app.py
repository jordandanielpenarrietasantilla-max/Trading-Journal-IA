...
st.set_page_config(page_title="Journaling & AI Trading Audit", page_icon="📈", layout="wide")


# --- APLICAR ESTILO CSS NEÓN Y GLASSMORPHISM ---
def aplicar_fondo_local(ruta_imagen):
    bg_style = ""
    if os.path.exists(ruta_imagen):
        with open(ruta_imagen, "rb") as image_file:
            encoded_string = base64.b64encode(image_file.read()).decode()
        bg_style = f'background-image: linear-gradient(rgba(11, 14, 20, 0.65), rgba(11, 14, 20, 0.75)), url("data:image/jpeg;base64,{encoded_string}") !important;'
...
# =====================================================================
# 1. LANDING PAGE Y LOGIN (PÚBLICO - SI NO HA INICIADO SESIÓN)
# =====================================================================
...
# =====================================================================
# 2. HERRAMIENTA PRIVADA COMPLETA (UNA VEZ INICIADA SESIÓN)
# =====================================================================
else:
    user_id = st.session_state.usuario_logueado.id
    user_email = st.session_state.usuario_logueado.email

    # Cargar Trades desde Supabase FILTRADOS POR USUARIO
    ...
    st.write(f"Bienvenido de nuevo. Mide tu progreso y disciplina en tiempo real.")
            ...
            if st.button("🚀 Guardar Trade Ejecutado"):
                ...
                if supabase:
                    try:
                        ...
                        st.success("¡Trade guardado en tu cuenta privada de forma permanente! ☁️")
                    except Exception as e:
        ...
        with st.expander("➕ Guardar Nueva Proyección Manualmente", expanded=False):
            ...
            with c_p1:
                ...
                if par_proy_sel == "Otro (Escribir manualmente)":
                    par_proy = st.text_input("✍️ Escribe el nombre del Activo:", placeholder="Ej. TSLA, NVDA...", key="par_proy_manual")
                else:
    ...
    with tabs[4]:
        ...
        with st.expander("✍️ Escribir Bitácora de Hábitos y Estado Mental del Día", expanded=True):
            ...
            bitacora_texto = st.text_area(
                "🧠 Espacio Libre: Reflexiones, hábitos cumplidos (sueño, ejercicio), emociones o lecciones aprendidas:",
                height=120,
                placeholder="Dormí 8 horas y estuve muy paciente. Esperé a que el precio llegara a mi FVG...",
                ...
                if bitacora_texto.strip() != "":
                    ...
                    if supabase:
                        try:
                            ...
                            st.session_state.trades.append(res.data[0])
                            st.success("¡Reflexión y Hábitos guardados exitosamente! 🧠⚡")
                            st.rerun()
                        except Exception as e:
                            st.error(f"Error guardando en Supabase: {e}")
                    else:
        ...
        if not trades_ejecutados:
            st.write("No hay entradas ejecutadas o bitácoras guardadas aún en tu cuenta.")
        else:

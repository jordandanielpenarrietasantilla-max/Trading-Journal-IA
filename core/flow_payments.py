...
def _commerce_order(*, user_id: str, plan_code: str) -> str:
    clean_plan = re.sub(r"[^A-Za-z0-9]", "", plan_code)[:16]
    clean_user = re.sub(r"[^A-Za-z0-9]", "", user_id)[:16]
    unique = uuid.uuid4().hex[:18]
    return f"AXION-{clean_plan}-{clean_user}-{unique}"[:80]
